...
document.addEventListener("DOMContentLoaded", function () {
  document
    .getElementById("placeSearchInput")
    .addEventListener("input", function (e) {
      displaySearchResults(e.target.value);
    });

  document
    .getElementById("envelopeIcon")
...
function displaySearchResults(query) {
  ...
  const results = fakePlaces.filter((place) =>
...
function showSuggestions(input) {
  let suggestions = "";
  if (input.length > 0) {
    const filtered = fakeFriends.filter((friend) =>
      friend.name.toLowerCase().startsWith(input.toLowerCase())
    );
    suggestions = filtered
      .map(
        (friend) =>
          `<div onclick="addTag('${friend.userID}')">${friend.name}</div>`
      )
      .join("");
  }
  document.getElementById("userSuggestions").innerHTML = suggestions;
  document.getElementById("userSuggestions").style.display = suggestions
    ? "block"
    : "none";
}
